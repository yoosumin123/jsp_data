...
public class BoardController extends HttpServlet {
	...
	private void doHandle(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if (action == null) {
			...
			} else if (action.equals("/modArticle.do")) { // 수정 반영하기 클릭 시 modArticle.do 요청
				Map<String, String> articleMap = upload(request, response);// 수정된 글정보와 첨부된 이미지파일이름을 articleMap으로 받음
				...
				articleVO.setContent(content);
				articleVO.setImageFileName(imageFileName);
				boardService.modArticle(articleVO); // boardService의 modArticle() 메소드로 articleVO를 전달.
				if (imageFileName != null && imageFileName.length() != 0) { // 수정 이미지를 첨부한 경우
					...
					oldFile.delete(); // 기존에 있던 이미지파일을 삭제한다
				}
				PrintWriter pw = response.getWriter();
